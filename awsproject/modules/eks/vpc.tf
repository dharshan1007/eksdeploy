...
resource "aws_vpc" "eks-vpc" {
  ...
  tags = {
    Name  = "${var.solution}-${var.environment}-vpc",
    "kubernetes.io/cluster/${var.solution}-${var.environment}-cluster" = "shared"
...
resource "aws_subnet" "eks-public" {
  ...
  tags = {
    Name = "${var.solution}-${var.environment}-public-subnet"
    "kubernetes.io/cluster/${var.solution}-${var.environment}-cluster" = "shared"
...
resource "aws_subnet" "eks-private" {
  ...
  tags = {
    Name = "${var.solution}-${var.environment}-private-subnet"
    "kubernetes.io/cluster/${var.solution}-${var.environment}-cluster" = "shared"
...
resource "aws_internet_gateway" "eks-igw" {
  ...
  tags = {
    "Name" = "${var.solution}-${var.environment}-igw"
  }
...
resource "aws_route_table" "main" {
  ...
  tags = {
    Name = "${var.solution}-${var.environment}-Default-rt"
...
resource "aws_eip" "main" {
  ...
  tags = {
    Name = "${var.solution}-${var.environment}-ngw-ip"
    environemnt = var.environment
    solution = var.solution
  }
}
...
resource "aws_nat_gateway" "main" {
  ...
  tags = {
    Name = "${var.solution}-${var.environment}-ngw"
    environemnt = var.environment
    solution = var.solution
  }
}
...
resource "aws_security_group" "eks_public_sg" {
  name   = "${var.solution}-${var.environment}-Public-sg"
  vpc_id = aws_vpc.eks-vpc.id
...
resource "aws_security_group" "data_plane_sg" {
  name   = "${var.solution}-${var.environment}-Worker-sg"
  vpc_id = aws_vpc.eks-vpc.id
...
resource "aws_security_group" "control_plane_sg" {
  name   = "${var.solution}-${var.environment}-ControlPlane-sg"
  vpc_id = aws_vpc.eks-vpc.id
